...
function repinto_botones()
{
	...
	//MAS Y MENOS 3
	if(PA<=3){document.getElementById("-").classList.add('disabled');}
	else{document.getElementById("-").classList.remove('disabled');}
	if(PA>=(NP-2)){document.getElementById("+").classList.add('disabled');}
	else{document.getElementById("+").classList.remove('disabled');}
}
//***********************************************************************
...
function cambio_3(simbolo)
{
	if(simbolo=="+"){
		...
		if(num<=0){
			...
			//PULSADOAHORA contiene el numero que al restar 5(paginas de vista)
			//menos 3(los que retrocedo)equivale a sumar 2 con la nueva tabla de navegacion hecha
			cualquiera(PulsadoAHORA+2);
		}else{cualquiera(num);}
	}
}
</script>
...
<body onload="inicio();">
	...
	<div id="cajapagina" class="contenedor2">
			...
			<div  class="paginacion" id="navegacion" style='border:0px solid red;margin-bottom:10px;margin-top:10px'>
					...
					<!-- 2 botones extra-->
					<a id="+" onclick="cambio_3('+')" href='#'>+3</a>
					<a id="-" onclick="cambio_3('-')" href='#'>-3</a>
