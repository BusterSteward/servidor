...
function ultima()
{
	 ...
	 // este if no hace falta porque "repinto_botones()" lo des-habilita
	 if (PA<NP)
	{
			// actualizo variables
			if (TROZO==0)
...
function borrar(celda){
	...
	var fila_pulsada=celda.parentNode.parentNode.rowIndex; //FILA
	var eldni=table.rows[fila_pulsada].cells[0].innerHTML; //DNI

	var url= "BajaUsuarios.php?DNI="+eldni;
	...
	.then(function(datos){
		//alert(datos)
		//Aqui en datos tengo un 1 si he borrado y un 0 si no he borrado nada
	})
	.catch(function(err)
	{
		alert(err);
...
function fetch_listado(REGI,REGF)
{
	...
	.then(function(datos)
	{
			...
			lista.forEach(elemento =>
			{
				...
				"<p>"+elemento.edad+"</p>"+
				"<p>"+elemento.precio+"</p>"+
				"<br>"+"<img width='45px' height='45px' style='margin-top:2px' src='data:image/jpeg;base64,"+elemento.imagen+"'>"+
				"<br>"+"<img onclick='borrar(this)' style='cursor:pointer;width='45px' height='45px' style='margin-top:2px' src='imagenes/papelera.png'>";
