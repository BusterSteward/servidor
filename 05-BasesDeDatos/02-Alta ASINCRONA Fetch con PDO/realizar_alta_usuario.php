...
// SALIDA: número de error -> devuelvo el número de error que se haya producido
//*****************************************
...
$foto_temporal= $_FILES['imagen']['tmp_name'];
 ...
 //$foto_reconvertida=addslashes(file_get_contents ($_FILES['imagen']['tmp_name'])); 
...
$consulta = $conexion->prepare("INSERT $tabla (DNI,NOMBRE,EDAD,FECHA, PRECIO, OBSERVACIONES,IMAGEN,BLOQUEO) 
						VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
if(!$consulta){
	echo $conexion->errorCode();
}
elseif(!$consulta->execute([$v1,$v2,$v3,$v4,$v5,$v6,$foto_reconvertida,'0'])){
	echo $consulta->errorInfo()[1];
}
else{
	echo 1;
}
