...
function inicio()
{
	...
	document.getElementById("boton3").classList.add('disabled');
	document.getElementById("boton4").classList.add('disabled');		
}
function estiloTabla(n){
	switch(n){
		case 1:
			TP=4;
			PA=1;
...
function borrar(boton){
	...
	let id=fila.firstChild.innerHTML;
	...
	fetch(url,{
		...
		switch(response){
			case 0:
				alert("El registro ya estaba borrado")
				break;
...
function fetch_listado(REGI,REGF)
{
	...
	.then(function(datos)
	{
			...
			//si esta variable es 1 entonces visualizo la tabla de la primera forma
			if(estiloTabla==1){
				...
				lista.forEach(elemento =>
				{
					...
					fila.insertCell(6).innerHTML = "<img width='45px' height='45px' style='margin-top:2px;cursor:pointer' src='imagenes/papelera.png' onclick='borrar(this);'>";
				});
			}

			//si esta variable es 2 entonces visualizo la tabla de la segunda forma
			if(estiloTabla==2){
				
				//la segunda forma no tiene cabeceras en la tabla
				var body = table.createTBody();

				//inserto las 2 filas de la tabla
				let f1 = body.insertRow(0);
				let f2 = body.insertRow(1);

				let index;

				//inserto celdad e introduzco los registros si todavia quedan en la lista
				for(let i=0;i<4;i++){
					for(let j=0;i<2;i++){
						index=i+j;
						if(j==0){
							...
							f1.insertCell(i).innerHTML=html;
						}
						if(j==1){
...
<div id="cajapagina" class="contenedor2">
		...
		<div class="contenedor3">
				<button class="boton" id="boton1" type="button"  style="margin:10px;"
					onclick="estiloTabla(1);primera_consulta();">
					<i class="fas fa-question"></i> crear tabla 1
				</button>
				<button class="boton" id="boton2" type="button"  style="margin:10px;"
					onclick="estiloTabla(2);primera_consulta();">
					<i class="fas fa-question"></i> crear tabla 2
				</button>
				<button class="boton" id="boton3" type="button"  style="margin:10px;"
					onclick="consulta(2,2);">
					<i class="fas fa-question"></i> consulto celda [2,2]
				</button>
				<button class="boton" id="boton4" type="button"  style="margin:10px;"
					onclick="modifico_celdad();">
					<i class="fas fa-question"></i> modifico celdas
				</button>
